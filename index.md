...
multiple raster file paths into
[`par_multirasters()`](https://docs.ropensci.org/chopin/reference/par_multirasters.md).
**Finally,** users run `par_*()` function with the configurations set
above to compute spatial variables from input data in parallel:
...
- Each of the `par_*` functions introduced above has `mirai` version
  ...
  work properly after creating daemons with
...
[DGGRID](https://github.com/sahrk/DGGRID) in
[`par_pad_grid()`](https://docs.ropensci.org/chopin/reference/par_pad_grid.md).
Users can utilize each grid system with a proper resolution to improve
the efficiency of spatial operations.
...
- Memory size: `max_cells_in_memory` argument value of
  [`exactextractr::exact_extract()`](https://isciences.gitlab.io/exactextractr/reference/exact_extract.html),
  raster resolution, and the number of layers in `SpatRaster` are
  multiplicatively related to the memory usage.

![](reference/figures/README-flowchart-raster.png)
...
![](reference/figures/README-flowchart-vector.png)
...
![](reference/figures/README-read-nc-1.png)
...
![](reference/figures/README-gen-ncpoints-1.png)
...
![](reference/figures/README-load-srtm-1.png)
...
takes a spatial dataset to generate regular grid polygons with `nx` and
`ny` arguments with padding. Users will have both overlapping (by the
degree of `radius`) and non-overlapping grids, both of which will be
...
![](reference/figures/README-compare-compregions-1.png)

From 0.9.5, H3 and DGGRID systems are supported in
[`par_pad_grid()`](https://docs.ropensci.org/chopin/reference/par_pad_grid.md):
...
![](reference/figures/README-h3-grid-1.png)
...
![](reference/figures/README-dggrid-grid-1.png)
...
locations, with 10 kilometers radius by
[`chopin::par_grid()`](https://docs.ropensci.org/chopin/reference/par_grid.md).
Users always need to **register** multiple CPU threads (logical cores)
for parallelization. `chopin::par_*()` functions are flexible in terms
of supporting generic spatial operations in `sf` and `terra`, especially
...
![](reference/figures/README-plot%20results-1.png)
...
![](reference/figures/README-plot%20results-2.png)
...
group of block groups.
[`chopin::par_hierarchy()`](https://docs.ropensci.org/chopin/reference/par_hierarchy.md)
leverages such hierarchies to parallelize geospatial operations, which
means that a group of lower-level geographic units in a higher-level
geography is assigned to a process. A demonstration below shows that
census tracts are grouped by their counties then each county will be
processed in a CPU thread.
...
[`chopin::par_multirasters()`](https://docs.ropensci.org/chopin/reference/par_multirasters.md)
is for such cases. An example below demonstrates where we have five
...
[`terra::nearest()`](https://rspatial.github.io/terra/reference/nearby.html),
which gets the nearest feature’s attributes, inside
[`chopin::par_grid()`](https://docs.ropensci.org/chopin/reference/par_grid.md).
...
#>   `/tmp/Rtmphl1eXq/ncroads_first.gpkg' using driver `GPKG'
...
![](reference/figures/README-map-all-1.png)
